...
def s_precision(p1):
    etat_precision = Precision(Precision.portee,
                               Precision.duree, Precision.duree)
    s_precision = Sort(p1,
                       conditions=[],
...
def test_precision_p2(terrain, p1, s1, s_precision, p2, s2):
    assert p1.x, p1.y == (5, 5)
    assert p2.x, p2.y == (6, 5)
    terrain.start()
    terrain.fin_de_tour()
    
    p2.lancer_sort(7, 5) # 1 case.
    assert p2.pa == 3
    assert p2.pm == 1
    terrain.fin_de_tour()
    terrain.fin_de_tour()

    p2.lancer_sort(8, 5) # 2 case.
    assert p2.pa == 6
    assert p2.pm == 3
    terrain.fin_de_tour()
